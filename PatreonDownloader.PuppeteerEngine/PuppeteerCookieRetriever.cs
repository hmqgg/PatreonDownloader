...
namespace PatreonDownloader.PuppeteerEngine
{
    public class PuppeteerCookieRetriever : ICookieRetriever, IDisposable
    {
        ...
        private readonly bool _isHeadlessBrowser;
        private readonly bool _isRemoteBrowser;
            ...
            try
            {
                ...
                if (browserCookies != null && browserCookies.Length > 0)
                {
                    foreach (var browserCookie in browserCookies)
                    {
                        ...
                        var cookie = new Cookie(browserCookie.Name, browserCookie.Value, browserCookie.Path, browserCookie.Domain);
        ...
        private async Task Login()
        {
            ...
            do
            {
                ...
                var response = await page.GoToAsync("https://www.patreon.com/api/current_user");
                if (response.Status == HttpStatusCode.Unauthorized)
                {
                    ...
                    if (_isRemoteBrowser)
                    {
                        await page.CloseAsync();
                        throw new Exception(
                            "You are not logged in into your patreon account in remote browser. Please login and restart PatreonDownloader.");
                    }
